...
last_round_result = {"result": "Waiting for round...", "multiplier": "0.00x", "color": "text-yellow-300"}
...
async def user_status(username: str):
    ...
    try:
        ...
        user = db.execute(text("SELECT balance, current_profit, joined_session FROM users WHERE username=:username"), {"username": username}).fetchone()
        if not user:
        ...
        history = db.execute(text("""SELECT result, bet_amount, profit_loss, timestamp
                      FROM bet_history WHERE username=:username ORDER BY id DESC LIMIT 10"""), {"username": username}).fetchall()
...
async def monnify_webhook(request: Request):
    ...
    try:
        ...
        db.commit()
        print(f"✅ Monnify deposit credited: ₦{amount} to {username}")
        ...
        <div class="max-w-7xl mx-auto p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <div class="bg-purple-900/30 border border-purple-500 p-6 rounded-3xl text-center">
                    ...
                    <p class="text-gray-400 mt-2">Total Platform Fees</p>
                </div>
